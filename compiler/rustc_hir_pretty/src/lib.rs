...
impl<'a> State<'a> {
    ...
    fn print_expr(&mut self, expr: &hir::Expr<'_>) {
        ...
        match expr.kind {
            ...
            hir::ExprKind::Struct(hir::LazyStruct::Finalized(qpath, fields, wth)) => {
                self.print_expr_struct(qpath, fields, wth);
            }
            hir::ExprKind::Struct(hir::LazyStruct::Inferred(..)) => panic!("Print expr called on non-finalized struct"),
